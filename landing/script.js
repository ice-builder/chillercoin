...
function setupReveal() {
    const selectors = '.step-card, .phil-card, .perf-stat-card, .perf-chart-card, .token-row, .rm-card, .engine-features li, .origin-card, .engine-card, .flow-step, .rev-card, .not-scam-card, .comp-group, .mwm-col, .mwm-quote';
    ...
    document.querySelectorAll('.token-row').forEach((el, i) => el.style.setProperty('--delay', `${i * 0.08}s`));
...
(function() {
    ...
    chart.style.transform = 'scale(0.5) rotate(-120deg)';

    const obs = new IntersectionObserver(entries => {
        if (entries[0].isIntersecting) {
            chart.style.transition = 'all 1.2s cubic-bezier(0.34, 1.56, 0.64, 1)';
