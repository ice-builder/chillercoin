...
v7.0: Data-driven optimization (2026-05-14)
  ...
  - Leverage stays at 5x (current params working well: +$443/day)
"""
import os
...
# ─── Auto-Entry Settings ───────────────────────
POSITION_SIZE_PCT = 5             # 5% of balance (base size)
POSITION_SIZE_HIGH_PCT = 7        # v7.0: 7% for score >= 22 (scale into high-conviction)
HIGH_SCORE_THRESHOLD = 22         # Score threshold for larger position sizing
LEVERAGE = 5                      # v2: 5x leverage (was 10x — halved after -35% drawdown)
MAX_POSITIONS = 3                 # Max 3 insider positions
ENTRY_DIRECTION = "auto"          # v2: auto-detect direction from price action (was "long" only)

# ─── Risk Management (NEW) ────────────────────
# Data: -3% hard stop turns -$39K into +$3.4K on 27 trades
HARD_STOP_PCT = -5.0              # v2: Hard stop at -5% (was -3% — too tight for alts at 5x)
TRAIL_ACTIVATION_PCT = 3.0        # v2: Activate trailing after +3% (was +5% — rarely reached)
TRAIL_STOP_PCT = 2.0              # v2: Trail 2% from peak (was 3%)
TIME_STOP_HOURS = 6               # v2: Close flat after 6h (was 4h — give more time)
